"""Package the four locked final deliverables after final-state validation."""
import json, hashlib, zipfile, struct
from pathlib import Path
OUT=Path(__file__).resolve().parents[1]
BASE=OUT.parent/'blockout_v02'
m=json.loads((OUT/'build_FINAL_metadata.json').read_text())
a=json.loads((OUT/'validation_results.json').read_text())
r=json.loads((OUT/'roundtrip_results.json').read_text())
freeze=json.loads((OUT/'spatial_freeze_manifest.json').read_text())
assert a['overall_status']=='pass' and a['summary']['fail']==0 and r['roundtrip_pass']
assert m['freeze_status']=='COMPOSITION_LOCKED'
for name,record in json.loads((BASE/'delivery_v02_manifest.json').read_text()).items():
...
assert struct.unpack('>II',(OUT/'hero_FINAL.png').read_bytes()[16:24])==(1536,1024)
c=m['final_camera_parameters']; g=a['details']['glb']
def v(values):return '('+', '.join(f'{x:.6f}' for x in values)+')'
compartment_rows=[]
for room in m['cabinet_compartments']:
    lo,hi=room['bounds_local']; size=[hi[i]-lo[i] for i in range(3)]
    proxy=room['proxies'][0]; bb=freeze['objects'][proxy]['world_bounds']
    ps=[bb[1][i]-bb[0][i] for i in range(3)]
    compartment_rows.append(f"| {room['name']} | {size[1]:.4f} × {size[0]:.3f} × {size[2]:.3f} | {ps[1]:.3f} × {ps[0]:.3f} × {ps[2]:.3f} | `{proxy}` |")
status="SHARKY'S ROOM — BLOCKOUT FINAL / COMPOSITION LOCKED"
report=f'''# BLOCKOUT_FINAL_REPORT — v0.2.1
...
| Position XYZ / m | {v(c['position_xyz_m'])} |
| Euler XYZ / degree | {v(c['rotation_xyz_degrees'])} |
| Quaternion WXYZ | {v(c['rotation_quaternion_wxyz'])} |
| Target XYZ / m | {v(c['target_xyz_m'])} |
| Sensor width × height | {c['sensor_width_mm']:.0f} × {c['sensor_height_mm']:.0f}mm，Sensor Fit={c['sensor_fit']} |
| Horizontal / vertical FOV | {c['horizontal_fov_degrees']:.6f}° / {c['vertical_fov_degrees']:.6f}° |
| Shift X / Y | 0 / 0 |
...
**正式冻结后完整验收：{a['summary']['pass']} PASS / {a['summary']['fail']} FAIL / {a['summary']['warning']} WARNING。**
...
- **{g['size_bytes']:,} bytes = {g['size_bytes']/1e6:.6f} MB**，小于5MB目标。
- **{g['triangles']:,} triangles**；{g['nodes']} nodes，{g['meshes']} meshes，{g['vertices']:,} exported vertices。
- 6个既有灰盒材质，0 textures / images，5个相机。
- glTF 2.0标准Y-up，保留原层级、独立节点、extras、相机与 `KHR_lights_punctual`。
- 冻结后的GLB在全新Blender场景中实际导入成功：**{r['reimported_objects']}/{r['source_objects']}对象**；名称、parent、origin、mesh bounding boxes、triangle count全部一致。
- 五个相机 world matrix 通过，最大误差 {max(cam['world_matrix_max_error'] for cam in r['cameras']):.3e}；GLB FOV / aspect / clip 同时通过检查。
...
'''
(OUT/'BLOCKOUT_FINAL_REPORT.md').write_text(report,encoding='utf-8')
m['validation_status']='FINAL_LOCKED_VALIDATION_PASSED'
m['final_validation_summary']=a['summary']
(OUT/'build_FINAL_metadata.json').write_text(json.dumps(m,indent=2,ensure_ascii=False))
names=['sharkys_room_blockout_FINAL.blend','sharkys_room_blockout_FINAL.glb','hero_FINAL.png','BLOCKOUT_FINAL_REPORT.md']
with zipfile.ZipFile(OUT/'sharkys_room_blockout_FINAL_delivery.zip','w',zipfile.ZIP_DEFLATED) as z:
    for name in names:z.write(OUT/name,arcname=name)
with zipfile.ZipFile(OUT/'sharkys_room_blockout_FINAL_delivery.zip') as z:assert z.testzip() is None
manifest={name:{'bytes':(OUT/name).stat().st_size,'sha256':hashlib.sha256((OUT/name).read_bytes()).hexdigest()} for name in names}
(OUT/'delivery_FINAL_manifest.json').write_text(json.dumps(manifest,indent=2))
print(json.dumps({'status':status,'validation':a['summary'],'glb_bytes':g['size_bytes'],
                  'triangles':g['triangles'],'zip_bytes':(OUT/'sharkys_room_blockout_FINAL_delivery.zip').stat().st_size,
                  'source_v02_unchanged':True,'outputs':names},ensure_ascii=False,indent=2))
