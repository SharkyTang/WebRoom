...
Two original wall assemblies keep their real door/window openings. Lower 85 mm bands become separate warm trim inside the original wall slab; the plaster solids have eased cutaway edges. The wall silhouette, height and all openings remain frozen.

All wall solids are reconstructed from the actual frozen per-component boxes. Splitting the lower band changes surface treatment without moving the contact plane or increasing slab thickness.
...
| --- | --- | --- | --- |
| VIS_WallLeft | ENV_Wall_Left | [-0.09,0,-2.9] | [0.09,3,2.9] |
| VIS_WallRight | ENV_Wall_Right | [-3.69,0,-0.09] | [3.69,3,0.09] |

## Material and UV

Warm plaster is metallic 0 / roughness .88; lower trim is 0 / .58. No image texture, baked lighting or new lights.
...
Source: blender-assets/walls_v06a.blend. Web delivery: public/models/production/walls_v06a.glb. From project root:

```sh
/Applications/Blender.app/Contents/MacOS/Blender --background --factory-startup --python-exit-code 1 --python scripts/assets/build_architecture_assets.py -- walls
```
...
Current exported totals: **1,296 triangles, 4 primitives, 2 materials, 0 embedded images, 39,828 GLB bytes**. Exact SHA256, per-root bounds, texture payload and source-component metadata are in asset-statistics.json. The primitive count is structural, not a renderer draw-call measurement.
