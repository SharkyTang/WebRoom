...
- 实际 GLB：**297,804 B**；可编辑 .blend：**148,410 B**。
- 4,888 三角面、4 网格、4 primitives、4 材质、0 嵌入图片。primitives 不等于实际渲染 draw calls。
- 本家族无相机、灯光源或动画；无外部纹理/缓冲 URI。GLB 文件字节不等于 HTTP 传输量、显存或帧时间。
- GLB SHA-256：`606b8012f1aa52f9b9be0a380e3adea31424b46d53bdeb42a5d302e10e399ae9`。
...
| 三角面统计口径 | 数量 |
| --- | ---: |
| Blender 可编辑源（`sourceTriangles`） | 5,016 |
| 清理后的最终 GLB（`triangles`） | 4,888 |
| 导出后剔除的严格零面积三角形 | 128 |
