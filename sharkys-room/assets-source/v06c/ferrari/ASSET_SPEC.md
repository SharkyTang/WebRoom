...
所有尺寸为 glTF Y-up 米，向量顺序为 X / Y / Z。原锚点 `DSP_Vehicle_Bounds`、父级 `DISPLAY_MODELS`、Bounds 和 A 柜板保持。根 `VIS_Ferrari` 的平移/旋转为零、scale=1；统一缩放与 90° Y 轴展示朝向烘焙在局部顶点中，不对根或单轴做拉伸。

| 项目 | 实际数据 |
| --- | --- |
| 原锚点世界位置 | -3.225 / 0.275 / 0.715 m |
...
| --- | --- | --- | --- | --- |
| `MAT_V06C_Ferrari_Body` | 0.68 / 0.027 / 0.019 / 1 | 0.3 | 0.34 | OPAQUE |
| `MAT_V06C_Ferrari_TyresCockpit` | 0.018 / 0.022 / 0.025 / 1 | 0.03 | 0.63 | OPAQUE |
| `MAT_V06C_Ferrari_Accent` | 0.8 / 0.79 / 0.71 / 1 | 0.32 | 0.38 | OPAQUE |
| `MAT_V06C_Ferrari_DisplayPlinth` | 0.044 / 0.055 / 0.062 / 1 | 0.2 | 0.49 | OPAQUE |

- 实际 GLB：**119,244 B**；可编辑 .blend：**132,377 B**。
...
- GLB SHA-256：`f5a24caa07a52d8168c991280f93cd675f52c63547671941d19457b1e48de5fa`。
- 实际字节/图片重复成本复核：`node scripts/assets/inspect-decor-budgets.mjs`；逐组账本留在 `validation/v06c/stages/`，不覆盖历史记录。

## 原锚点装配与完整回退

- `VIS_Ferrari` → 原 `DSP_Vehicle_Bounds`；proxy=该原锚点中实际 mesh（不隐藏整个父组）。
...
- 源：`blender-assets/ferrari_v06c.blend`；导出：`public/models/production/ferrari_v06c.glb`。
- 生成记录：本目录 `asset-statistics.json`；包含实际导出 SHA、根 identity、包络和逐项往返检查。
- 重新生成本家族：`node scripts/assets/build-collection-decor.mjs ferrari`。该命令会重新写本家族 C 源、GLB 与统计，执行前按任务要求保留快照；无需安装依赖。
- 更新本规格：`node assets-source/v06c/write-specs.mjs --update ferrari`。仅从当前已导出并登记的对应 C 家族读取，拒绝 GLB/统计 SHA 不一致。
