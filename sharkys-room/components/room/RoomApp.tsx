...
export default function RoomApp() {
  ...
  return <main className="room-app" data-room-status={status} data-interaction-phase={interaction.interactionPhase} onClick={event => {
    // Canvas gestures have their own raycast route. Panels and controls keep
    // their normal actions; unhandled page/stage background shares Back.
    const target = event.target;
    if (target instanceof Element && !target.closest('canvas, .canvas-frame, .interaction-overlay, .debug-panel, button, a, input, select, textarea, label, [role="button"]')) void store.sceneClick(null);
  }} {...Object.fromEntries(assetFamilies.map(id => [`data-asset-${id}`, assets[id].status]))}>
