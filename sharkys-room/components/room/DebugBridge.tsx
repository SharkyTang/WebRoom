...
export type DebugSnapshot=Pick<RoomCanvasProps,'status'|'validation'|'hovered'|'selected'|'performance'> & {camera:{name:string;position:number[];quaternion:number[];fov:number;aspect:number;near:number;far:number}|null};
...
export function DebugBridge(props:RoomCanvasProps & {room:Group|null}) {
  ...
  useEffect(()=>{
    ...
    const api={snapshot:():DebugSnapshot=>{const p=latest.current;const camera=get().camera;return {status:p.status,validation:p.validation,hovered:p.hovered,selected:p.selected,performance:p.performance,camera:camera instanceof PerspectiveCamera?{name:camera.name,position:camera.position.toArray(),quaternion:camera.quaternion.toArray(),fov:camera.fov,aspect:camera.aspect,near:camera.near,far:camera.far}:null};},
