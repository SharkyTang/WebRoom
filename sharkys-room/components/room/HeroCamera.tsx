...
export function HeroCamera({source}:{source:PerspectiveCamera}) {
  ...
  const hero=useMemo(()=>{
    ...
    (clone as PerspectiveCamera & {manual:boolean}).manual=true;
    clone.aspect=HERO_ASPECT; clone.updateProjectionMatrix(); clone.updateMatrixWorld();
    return clone;
  },[source]);
  useLayoutEffect(()=>{const previous=get().camera;set({camera:hero});return()=>{set({camera:previous});};},[get,set,hero]);
  return null;
}
