...
export function DebugPanel({ validation, performance, interaction, ready, assets }: { validation: RoomValidation | null; performance: RoomPerformance | null; interaction: InteractionState; ready: boolean; assets: AssetLoadReport }) {
  return <details className="debug-panel"><summary>DEV · {validation?.ok ? '85 source nodes verified' : 'Validating'} · {performance ? `${performance.fps.toFixed(0)} fps` : 'sampling…'}</summary>
    <div><p>phase: {interaction.interactionPhase} · active: {interaction.activeObject ?? "—"}<br/>hovered: {interaction.hoveredObject ?? "—"} · cameraBusy: {String(interaction.isCameraBusy)}<br/>pianoState: {interaction.pianoState}<br/>pianoRetractedHitAreaActive: {String(ready && isPianoRetractedHitAreaActive(interaction))}<br/>macbook: {interaction.macbookState}<br/>trash: {interaction.trashState} · lights: {interaction.lightsState}<br/>marshall: {interaction.marshallPower} · time: {interaction.time} · weather: {interaction.weather}</p><p>Visuals: {Object.entries(assets).map(([id, value]) => `${id}: ${value.status}`).join(' · ')}</p><p>Hero: CAM_Hero_FINAL · 48mm · 3:2</p><p>{validation?.nodeCount ?? 0} source nodes · {validation?.meshCount ?? 0} primitive meshes · {validation?.triangleCount ?? 0} triangles</p>
