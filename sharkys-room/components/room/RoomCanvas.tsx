...
export type RoomCanvasProps = {
  debug: boolean; status: RoomLoadStatus; validation: RoomValidation | null; hovered: InteractionId | null; selected: InteractionId | null; performance: RoomPerformance | null;
...
export default function RoomCanvas(props: RoomCanvasProps) {
  return <CanvasBoundary onError={props.onError}><Canvas dpr={[1, 1.5]} frameloop={props.debug ? 'always' : 'demand'} gl={{antialias:true, powerPreference:'low-power', toneMapping:AgXToneMapping, outputColorSpace:SRGBColorSpace}} onPointerMissed={() => {props.onHover(null); props.onSelect(null);}} onPointerLeave={() => props.onHover(null)} fallback={<p role="alert">WebGL 2 is unavailable in this browser.</p>}>
