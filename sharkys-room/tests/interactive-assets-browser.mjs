...
const { assetManifest, assetFamilies, decorFamilies = [] } = await tsImport('../lib/room/assets/assetManifest.ts', import.meta.url);
...
const authorizedC = ['eiffel', 'hogwarts', 'minastirith', 'falcon', 'bridge', 'sls', 'ferrari', 'mercedes', 'plants', 'cola', 'dog', 'fixtures', 'wallart'];
for (const family of decorFamilies) assert(authorizedC.includes(family), `Unapproved C family: ${family}`);
assert.equal(new Set(decorFamilies).size, decorFamilies.length, 'C registration cannot duplicate families');
assert.deepEqual([...assetFamilies].sort(), [...expectedOld, ...bFamilies.filter(family => assetFamilies.includes(family)), ...decorFamilies].sort(), 'Only protected old three/A16 and explicitly authorized B/C families may be installed');
