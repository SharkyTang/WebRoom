...
const baselineMode = process.env.ROOM_B_PERFORMANCE_BASELINE === '1';
const output = process.env.ROOM_TEST_OUTPUT ?? path.join(root, 'validation/v06b', baselineMode ? 'baseline' : 'performance');
const baselinePath = process.env.ROOM_B_PERFORMANCE_BASELINE_FILE ?? path.join(root, 'validation/v06b/baseline/performance.json');
...
const oldFamilies = ['monitor', 'macbook', 'marshall', 'floor', 'walls', 'door', 'window', 'curtains', 'desk', 'cabinet', 'bed', 'bedside', 'sofa', 'chair', 'coffee', 'sidetable', 'beanbag', 'rugs', 'dogbed'];
...
if (baseline) assert.equal(baseline.summary.failed, 0, 'Comparison requires a successful current-session baseline');
...
async function activity(page, label, action) {
  await page.evaluate(() => {
    window.__v06bFrames = []; window.__v06bSampling = true;
    function sample() {
      const state = window.__ROOM_DEBUG__.snapshot();
      window.__v06bFrames.push({ atMs: performance.now(), renderFrames: state.renderFrames, phase: state.interaction.interactionPhase, cameraBusy: state.interaction.isCameraBusy, activeObject: state.interaction.activeObject, pianoState: state.interaction.pianoState, macbookState: state.interaction.macbookState, trashState: state.interaction.trashState });
      if (window.__v06bSampling) requestAnimationFrame(sample);
    }
    sample();
  });
  try { await action(); } finally { await page.evaluate(() => { window.__v06bSampling = false; }); }
  const frames = await page.evaluate(() => window.__v06bFrames), intervals = [];
...
try {
  ...
  if (baseline) await check('Same-condition B delta with unchanged pre-B exports', async () => {
    for (const old of baseline.modelFiles) assert.deepEqual(modelFiles.find(model => model.url === old.url), old, `Pre-B export changed: ${old.url}`);
