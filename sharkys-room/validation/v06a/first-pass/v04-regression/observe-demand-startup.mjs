...
try{
 ...
 await fs.writeFile(new URL('./startup-observation.json',import.meta.url),JSON.stringify({note:'Read-only RAF timeline after old ready condition; no input or application state mutation.',samples},null,2)+'\n');let key='';for(const s of samples){const next=JSON.stringify([s.frames,s.memory,s.buffer,s.canvas.width,s.canvas.height,s.textureUpdates]);if(next!==key){console.log(JSON.stringify(s));key=next;}}
