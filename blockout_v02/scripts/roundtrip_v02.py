...
def main():
    ...
    glb = args.glb or out / "sharkys_room_blockout_v02.glb"
    before = snapshot()
    ...
    passed = not (missing or extra or structure or origins or bounds or triangles) and len(cameras) == 4 and all(c["pass"] for c in cameras)
    ...
    result = out / "roundtrip_v02_results.json"
    result.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print("V02_ROUNDTRIP_RESULT " + json.dumps(report, ensure_ascii=False))
    if not passed:
